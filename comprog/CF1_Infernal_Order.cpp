...
void burn(string currentCityName,string previousCityName,int firePower){
    ...
    */
    if (firePower - cityStrength[currentCityName] <= 0) return;
    destroyedCities.insert(currentCityName);
    for (auto u : adjacentCities[currentCityName]) {
        if (u != previousCityName) {
            burn(u, currentCityName, firePower - cityStrength[currentCityName]);
        }
    }
}
int main(){
